...
/**
 * Les rubriques qu'un utilisateur peut consulter, et celles auxquelles il
 * peut répondre. Une seule requête, là où interroger rubrique par rubrique
 * en ferait six à chaque ouverture de la boîte.
 *
 * Renvoie deux tableaux plutôt qu'un seul : l'interface doit pouvoir montrer
 * une demande sans proposer d'y répondre, ce que le §21 distingue
 * explicitement (Lecture seule voit tout, n'écrit rien).
 */
export async function rubriquesAccessibles(db, utilisateur, toutesLesRubriques) {
  const { role, id: utilisateurId } = utilisateur
  if (role === 'super_admin' || role === 'coordination') {
    return { consulter: [...toutesLesRubriques], repondre: [...toutesLesRubriques] }
  }
  ...
  const lignes = await db
    .select()
    .from(rubriquePermission)
    .where(or(eq(rubriquePermission.role, role), eq(rubriquePermission.utilisateurId, utilisateurId)))

  const consulter = []
  const repondre = []
  for (const cle of toutesLesRubriques) {
    const pourCetteRubrique = lignes.filter((l) => l.rubriqueCle === cle)
    // Une exception individuelle prime sur la règle de rôle.
    const ligne =
      pourCetteRubrique.find((l) => l.utilisateurId === utilisateurId) ||
      pourCetteRubrique.find((l) => l.role === role)
    if (!ligne) continue
    if (ligne.peutConsulter) consulter.push(cle)
    if (ligne.peutRepondre) repondre.push(cle)
  }
  return { consulter, repondre }
}
