 ...
 * @param {object} [config.reference]        données de référence jointes aux réponses
  ...
  return async function handler(req, res) {
    ...
    if (req.method === 'GET') {
      ...
      if (id) {
        ...
        const historique = await db
          ...
          .limit(20)
        return res.status(200).json({ [entite]: ligne, historique, ...reference })
      }
      ...
      return res.status(200).json({ [nomPluriel]: lignes, ...reference })
    }
    ...
    if (req.method === 'POST') {
      ...
      const [creee] = await db
        ...
        .values({
          ...donnees,
          ...(preparer ? preparer(donnees, { avant: null, statutVise }) : {}),
          slug,
          statut: statutVise,
    ...
    if (req.method === 'PUT') {
      ...
      const [apres] = await db
        ...
        .set({
          ...donnees,
          ...(preparer ? preparer(donnees, { avant, statutVise }) : {}),
          slug,
          statut: statutVise,
          modifieParId: utilisateur.id,
