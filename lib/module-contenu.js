...
export function creerModuleContenu(config) {
  ...
  return async function handler(req, res) {
    ...
    if (req.method === 'POST') {
      ...
      // Le slug est calcule d abord : c est lui qui dit si le contenu est
      // protege (§8.4). Le controle de transition qui suit en depend, sans
      // quoi une page juridique pourrait etre creee directement publiee.
      const slug = await slugDisponible(slugifier(req.body?.slug || donnees[champTitre]))
      const estProtege = protege({ ...donnees, slug })

      const transition = transitionAutorisee('brouillon', statutVise, {
        peutPublier,
        toujoursAValider: estProtege,
      })
      ...
      // La protection se verifie aussi a la CREATION, pas seulement a la
      // modification : sans cela, un profil non habilite pourrait creer une
      // page dont le slug tombe sur celui d une page juridique (§8.4), puis
      // se retrouver a l avoir creee sans pouvoir la corriger.
      if (estProtege && !verifierDroit(utilisateur.role, "pages_protegees.modifier")) {
        return res.status(403).json({
          error: "Cette page engage l association sur le plan juridique. Seules la Coordination et le Super administrateur peuvent la creer ou la modifier.",
        })
      }
